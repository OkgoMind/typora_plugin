...
test("i18n testing suite", async (t) => {
    ...
    const testPromises = filesToTest.map(file => {
        return t.test(`should match structure and order of ${baseFile.name}: ${file.name}`, () => {
            const errors = []
            compareStructure(baseFile.obj, file.obj, [], errors)
            assert.strictEqual(errors.length, 0, `Structure mismatch found for ${file.name} (vs ${baseFile.name}):\n  - ${errors.join("\n  - ")}`)
        })
    })
